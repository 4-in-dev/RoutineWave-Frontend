...
export const updateElementFromArray = (arr, selectedJob) => {
  ...
  arr[selectedJob.seriesId][CONSTANT.INDEX_OF_STARTTIME] = selectedJob.data.start_time
    .split(":")
    .splice(0, 2)
    .join("");
  ...
  if (jobs.length !== 0) {
    if (jobs.length > 1) {
      for (let i = 0; i < jobs.length - 1; i++) {
        let timeGap =
          jobs[i + 1][CONSTANT.INDEX_OF_STARTTIME] - jobs[i][CONSTANT.INDEX_OF_STARTTIME];
      ...
      series.push({
        name: jobs[0][CONSTANT.INDEX_OF_CONTENT],
        data: jobs[0][CONSTANT.INDEX_OF_STARTTIME],
      });
    }
  }
  return series;
...
export const getJobsDateForMonth = (allJobOfMonth) => {
  const jobDateList = allJobOfMonth.map((job) => job.start_date);
  return Array.from(new Set(jobDateList));
};
